...
def filter_clients_by_mailing(client_queryset, client_filter: dict):
    ...
    if mobile_code and tag:
        return client_queryset.filter(
            Q(mobile_operator_code=mobile_code) | Q(tag=tag)
        )
    elif mobile_code:
...
def is_within_time_interval(client_timezone: str, time_interval: str):
    ...
    interval_start = datetime.combine(current_time_utc, interval_start).astimezone(pytz.UTC)
