...
def sending_mail():
    active_mailing = [
        mailing.id for mailing in Mailing.objects.all() if mailing.is_active
    ]
    messages_pull = Message.objects.filter(
        mailing_id__in=active_mailing, send_status=Message.MessageStatus.NOT_SENT
    )
